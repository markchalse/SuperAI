...
height = 720
param = 0.7
workspace = r"F:\majun\img\traj_img\train\isosceles_right_triangle"
...
def irt_vertices2list(vertices):
    x= []
    y = []
    for _ in vertices:
        x.append(_[0])
...
if __name__ == '__main__':
    ...
    for i in range(MAX_NUM):
        ...
        for theta_split in range(12):
            ...
            x,y =  irt_vertices2list(rotated_vertices)
            x,y = inside_points(x,y,40)
            img = pil_draw_pic(width,height,x,y)
            img = img[:, :, ::-1]  
            cv2.imwrite(f'{workspace}/irt_{pic_num}.jpg',img)
            pic_num += 1 
