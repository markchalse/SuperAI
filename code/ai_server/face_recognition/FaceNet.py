...
def resize_image(image, size, letterbox_image):
    #print ("66666666666666666666  ",image.size)
    iw, ih  = image.size
    #iw  =   image.shape[1]
    #ih  =   image.shape[0]
    w, h    = size
...
class FaceNetMatch(object):
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        #"model_path"    : r"D:\ai_space\model_endpoints\face_recognition\facenet_mobilenet.pth",
        "model_path"    : r"D:\ai_space\model_endpoints\face_recognition\facenet_inception_resnetv1.pth",
        #--------------------------------------------------------------------------#
        #   输入图片的大小。
        #--------------------------------------------------------------------------#
    ...
    def __init__(self, **kwargs):
        self.env = EnvConfig()
        self._defaults['model_path'] = self.env.facenet_model_path
    ...
    def get_image_feature(self,img):
        ...
        with torch.no_grad():
            image_1 = resize_image(img, [self.input_shape[1], self.input_shape[0]], letterbox_image=self.letterbox_image)
            
            photo_1 = torch.from_numpy(np.expand_dims(np.transpose(preprocess_input(np.array(image_1, np.float32)), (2, 0, 1)), 0))
            #photo_1 = torch.from_numpy(np.expand_dims(np.transpose(preprocess_input(np.array(img, np.float32)), (2, 0, 1)), 0))
